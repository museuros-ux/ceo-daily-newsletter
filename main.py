...
CATEGORIES = {
    "의왕시 동향": ["의왕시"],
    "지방공사 동향": ["도시공사", "지방공사", "지방공기업"],
    "지방공단 동향": ["시설관리공단", "지방공단"],
    "경영평가 동향": ["경영평가"],
    "개발 동향": ["도시개발", "도시재생", "택지개발", "재개발", "재건축"],
    "CEO 동향": ["도시공사 사장", "시설공단 이사장", "지방공기업 대표", "공사 CEO"],
}

SECONDARY_FILTER = {
    "지방공단 동향": ["공단", "시설관리공단", "지방공단"],
    "경영평가 동향": ["공기업", "지방공사", "지방공단", "도시공사", "시설공단", "공단", "공사", "지방공기업"],
}

CEO_TITLE_KEYWORDS = ["사장", "CEO", "이사장", "대표이사", "최고경영자"]
...
def get_nearby_json(nearby_articles, date_label):
    ...
    prompt = (
        ...
        "{\n"
        '  "인근도시공사": [\n'
        '    {"기관명": "안양도시공사", "제목요약": "핵심내용 20자 내외 요약 (주체 생략)", "동향": ["꼭지1", "꼭지2"], "출처": [{"제목": "기사제목", "링크": "기사링크"}]}\n'
        "  ]\n"
        "}\n\n"
        "작성 기준:\n"
...
def get_insight_json(news_data, date_label):
    ...
    prompt = (
        ...
        "- 각 꼭지는 명사형 또는 단문으로 끝낼 것. 서술식 금지\n"
        "- 개발 동향: 지자체·도시공사·개발공사가 시행하는 개발사업 관련 기사만 포함. 민간 개발·부동산 투자 기사 제외\n"
        "- 지방공단 동향: 무연고자 장례·복지 서비스 등 본연 업무와 무관한 기사 제외\n"
        "- 관련 기사 없으면 빈 배열 []\n"
...
def build_html(nearby_json, insight_json, date_label):
    ...
    def render_nearby(articles):
        ...
        for item in articles:
            ...
            html += (
                ...
                + ("[" + org_name + "] " if org_name else "") + title_summary +
                "</div>"
    ...
    content_html += render_nearby(nearby_json.get("인근도시공사", []))
    content_html += "</div>"
    num += 1

    remaining = ["지방공사 동향", "지방공단 동향", "경영평가 동향", "개발 동향", "CEO 동향"]
